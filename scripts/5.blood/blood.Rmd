---
title: "Visualize and test blood variables"
...
# Introduction
I will visualize and test blood variables.
...
meta <- "~/IKMB/projects/sport/data.clean/sampling.data.tsv" %>% 
  read_tsv()
blood <- "~/IKMB/projects/sport/data.clean/blood.tsv" %>% 
  read_tsv()
```
...
do.w.test <- function(data, test, group){
  ...
  number.of.units <- df %>% select(Unit) %>% unique() %>% nrow()
  if(number.of.units > 1){
    print(
      paste("Warning: more than one unit to be tested in", test, group))}
...
```{r}
groups <- blood %>% pull(Group) %>% unique()
tests <- blood %>% pull(Test) %>% unique()
blood
```
...
### Save plot elements
```{r}
save.figure.elements <- list(blood, label.df)
"figure.elements.rds" %>% 
  paste0(d.out, "/", .) %>% 
